...
- 粘贴文本 → 服务端 DeepSeek 抽出（无 key / 超时 / 坏 JSON 则规则/mock）→ 人工校对后入库。保存时走 `resolveIngredients` 自动关联或建食材档案，只有真正拿不准才问一句
...
- 加入库存、清单入库也走同一套解析：写下名字就会自动关联或建档案，只有拿不准才问一句；入库后回填食谱里还没关联的同名食材。过期日按分类默认保质期建议，过期可一键清掉。食材档案在库存页「更多」里，给要改别名或默认保质期的人用
